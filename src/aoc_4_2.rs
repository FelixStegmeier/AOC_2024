    ...
    for row_index in 1..matrix.len() - 1 {
        for col_index in 1..matrix[0].len() - 1 {
            if matrix[row_index][col_index] == search_for_string[1] {
                sum += search_cross_shape(matrix.clone(), row_index.clone(), col_index.clone());
                print!("Summe: {}", sum);
            }
        }
    }
...
fn search_cross_shape(matrix: Vec<Vec<char>>, row_index: usize, col_index: usize) -> i32 {
    let a =
        matrix[row_index - 1][col_index - 1] == 'M' && matrix[row_index + 1][col_index + 1] == 'S';
    let b =
        matrix[row_index + 1][col_index + 1] == 'M' && matrix[row_index - 1][col_index - 1] == 'S';
    let c =
        matrix[row_index - 1][col_index + 1] == 'M' && matrix[row_index + 1][col_index - 1] == 'S';
